...
大型园区、算力中心和工业微电网的调度失败，通常不是因为“不会算一条 96 点曲线”，而是因为现实运行在持续
变化：天气突变会让光伏预测失效，生产订单会临时改变负荷，分时电价和需量电费会放大错误决策，储能 SOC、
变压器温度、并网功率和关键产线保供又给调度加上硬约束。

传统 EMS 或人工排班常见的问题是：计划、预测、电价、设备状态和审批记录分散在不同系统里；旧计划是否仍然
有效无人持续判断；调度、审核、审批和执行边界不清；执行后也缺少实际回读来证明“省了多少钱、有没有违反
安全约束、是否误用了旧审批”。这些问题会带来真实代价：高峰错付、光伏弃电、储能过充过放、关键负荷误削、
生产返工、设备热风险和审计不可追溯。

EnergyMesh 要解决的是一条完整业务闭环：从真实园区数据或授权回放数据进入系统开始，持续判断当天调度计划
是否仍有效；一旦外部条件改变，就废止旧计划、重新规划、独立审核、触发人工审批、执行回读，并把每次决策
沉淀到按日期组织的运行账本中。任务完成条件不是“生成一份建议”，而是能回答：这一时段用了哪版预测、电价
和约束，哪个 Agent/Skill 产生了方案，谁批准了它，是否执行，执行后是否达到预期收益。

## 我们是如何解决的

EnergyMesh Agents 采用 AgentTeams 作为协同底座，而不是把多个函数固定串成九步流程。AgentTeams 在系统中
承担任务组织和责任分离：
...
复赛报告应使用大型园区数据重新给出：计划刷新时间、失效计划误执行数、约束违规数、执行结果回读率、相对
基线节省金额、光伏/清洁能源消纳率、人工介入次数和跨园区迁移后的策略筛选结果。
...
真实园区接入后，首页不是“宣传卡片”，而是按日期组织的运行账本：
