...
**EnergyMesh 是工业能源领域的系统工程平台。** 具备**云—边—端稳定部署架构**、**五类异常工程化降级状态机**和**八层确定性控制计算链**，面向真实工业能源场景的完整 AI 系统。



## 我们想解决什么问题

最重要的是节能，和高效灵活地响应用户需求。
...
## 核心技术：Physics + AI 混合建模（PhyAI）

EnergyMesh 的底层基于 **机理 → 数据 → 深度 → 控制** 的四阶闭环，让 AI 理解设备并产生可执行的节能控制动作。

```
机理 → 数据 → 深度 → 控制
定义边界   校准现实   学习复杂关系   产生实际价值
```

### 机理：物理边界先行

先知道设备**为什么这样运行**。基于热力学、流体动力学、电气特性和设备边界条件，为主机、水泵、冷却塔等关键设备搭建基础模型框架，还原其真实性能边界和运行逻辑。这一步定义了模型的可行域——什么运行状态在物理上是可能的，什么是绝对不可突破的安全边界。

### 数据：现场校准现实

再用真实运行数据校准理论模型。接入现场 EMS/BMS/PCS 历史数据，对机理模型的参数进行修正，让虚拟模型贴近真实现场。没有这一步，机理模型只是教科书公式；有了数据校准，才具备工程可用性。

### 深度：AI 学习复杂关系

让 AI 学习设备在**复杂工况下的非线性关系、耦合关系和误差项**。AI 持续自学习迭代，把模型误差压到 **RMSE＜3%**，让虚拟模型可以精准预测设备在不同工况下的能耗和出力表现。这一步处理的是机理模型无法表达的隐性关联，例如多设备耦合运行时的非线性损耗、环境变化带来的参数漂移等。

### 控制：计算成本降低

**控制的目标是下发设定值并计算成本降低。** 基于精准的设备数字镜像，系统调用优化器进行全局寻优，在满足生产需求和安全约束的前提下，直接生成可执行的功率设定、启停策略和储能充放电计划。每一次调度都必须量化成本节省——购电成本下降多少、能源浪费减少多少、人工调度工作量降低多少。

> **一句话概括：机理定义边界，数据校准现实，AI 学习复杂关系，控制产生实际价值。**

### 数字镜像：给每台关键设备建一个"算命"模型

用 PhyAI 方法，我们为每台关键设备建立一个数字镜像：

1. **先拿设备本身物理机理**（热力学、流体）搭基础模型框架，还原主机、水泵、冷却塔这类设备本身的性能边界、运行逻辑；
2. **接入现场真实运行数据**，用历史数据做参数校准修正；
3. **AI 持续自学习迭代**，把模型误差压到 RMSE＜3%，让虚拟模型可以精准预测设备在各种工况下的能耗、出力表现，给后续系统全局寻优节能做基础。

简单说：给每台关键设备建一个数字镜像，能算出来它在各种工况下会怎么耗电干活。

### PhyAI 的核心优势

物理机理提供可解释性和安全边界，AI 负责填补机理无法精确描述的复杂非线性。两者结合，既有可信度又有预测精度，最终输出的控制指令才真正具备下发到设备的工程可靠性。

### 成本导向的设计原则

EnergyMesh 的一切建模和优化，最终必须回答一个问题：**这次调度能为园区节省多少能源成本？**

- 购电成本（峰谷平电价套利、需量控制）
- 能源浪费（光伏弃光、储能过放、设备低效运行）
- 人工调度成本（减少重复排班、降低人工判断失误）
...
EnergyMesh 有三个工程化的核心支柱，Agent 协作承担最顶层的协同治理：
...
**Agent 在发现层和协调层工作。** 物理设定值由优化器和安全规则引擎确定性计算，Agent 负责在条件变化时触发重新评估、在异常时通知人、在需要审批时组织流程。即使 AgentTeams 或 LLM 掉线，边缘主机上的优化器和本地调度器仍然可以基于最后一版可信策略继续安全运行。
...
> **核心原则：Fail-safe。** 每一个子系统都有独立降级策略。

### 数据方案：八大分层 + 八层确定性控制链

数据分八层：Telemetry（实时遥测）、Asset Model（静态参数）、Forecast、Tariff、Constraint、Command、Readback、Derived State（派生状态）。
...
AgentTeams Element 是原生协作/证据界面；EnergyMesh UI 是面向园区能源运营的主界面。两者必须看到同一条真实任务链：
...
EnergyMesh 白色界面运行在本机或 Codespace 都可以，作为 Matrix/AgentTeams 业务客户端。
...
AgentTeams 自带的 Element 页面是证据/协作聊天室。正常演示请同时开两个入口：
...
| --- | ---: | ---: | --- |
| 24 小时模拟成本 | ¥15,616.85 | ¥13,628.21 | 真实回放测试中相对原 EMS 基线节省 ¥1,988.64，降低 12.7%；证据见 `baseline_vs_agent_savings.csv` |
...
指标用于故障定位、版本晋级、回滚和业务价值评估。原始样例可通过
...
本项目保留 MCP、RAG、数据库和云产品的等价机制和迁移成本说明：上层 Agent/Skill 契约保持不变，
