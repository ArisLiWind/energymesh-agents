...
EnergyMesh Agents 的目标是把「定制策略开发」变成「Agent 辅助策略生成与验证」：
...
> EnergyMesh Agents 是面向园区微电网的多 Agent 策略运行基础设施，让储能调度从定制代码走向可验证的策略自动化。
...
- 同一事件下对比人工规则基线和 EnergyMesh Agents 多 Agent 系统。
